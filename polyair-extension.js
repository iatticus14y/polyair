// polyair-extension.js - Gandi IDE Extension
class PolyairExtension {
    constructor(runtime) {
        ...
        this.glCanvas = null;
        this.gl = null;
        this.resolution = { width: 480, height: 360 }; // Default 480p
            ...
            blocks: [
                ...
                '---',
                {
                    ...
                    arguments: {
                        ...
                        B: { type: 'number', defaultValue: 255 }
                    }
                },
                {
                    opcode: 'setPenWidth',
    ...
    enableGPURendering(args, util) {
        ...
        console.log(`GPU rendering enabled for sprite: ${util.target.getName()}`);
    }

    disableGPURendering(args, util) {
        const targetId = util.target.id;
        this.gpuSprites.delete(targetId);
        console.log(`GPU rendering disabled for sprite: ${util.target.getName()}`);
    }
    ...
    setResolution(args) {
        ...
        console.log(`Resolution set to ${quality}: ${this.resolution.width}x${this.resolution.height}`);
    }

    setPenColor(args) {
        const r = Math.max(0, Math.min(255, parseFloat(args.R))) / 255;
        const g = Math.max(0, Math.min(255, parseFloat(args.G))) / 255;
        const b = Math.max(0, Math.min(255, parseFloat(args.B))) / 255;
        
        this.penColor = { r, g, b, a: 1.0 };
    }

    setPenWidth(args) {
        this.penWidth = Math.max(1, parseFloat(args.WIDTH));
    }
...
// Register extension with Gandi IDE
